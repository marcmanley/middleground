...
- **Standard sitewide image (use on every page, no exceptions):** `media/images/middle-ground-social-media-preview.png` with alt `Middle Ground &mdash; a space for prayer, community, and learning`. This is a dedicated branded social-card graphic (1700×893, ~1.9:1), not a page photo — do not substitute a page's own hero/featured image, even when one exists at a usable landscape ratio. The goal is a consistent, recognizable "Middle Ground" preview on every link share (WhatsApp, iMessage, Slack, etc.), not a page-specific thumbnail.
- `og:image:alt`/`twitter:image:alt` are therefore fixed sitewide copy tied to the standard image above, not reused from page content — this is the one field in this block that does not come from existing on-page text.
