...
**No structured data exists anywhere on the site.** Verified by searching all 25 HTML files for `application/ld+json`, `schema.org`, and microdata attributes (`itemscope`, `itemtype`): zero matches. There is also no Open Graph (`og:*`) or Twitter Card (`twitter:card`) metadata on any page — those aren't schema.org markup, but they're the other structured-metadata layer search/social crawlers read, so it's noted here for completeness (see [seo-roadmap.md](seo-roadmap.md)).

| Schema type | Pages found on | Generated by |
|---|---|---|
| *(none)* | *(none)* | *(n/a — nothing to audit)* |

Since there's nothing implemented, this document is entirely **recommendations** — none of it should be read as documenting an existing system.
